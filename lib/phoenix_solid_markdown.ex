defmodule PhoenixSolidMarkdown do
  @moduledoc """
  ...
  * `:except` Only files that do NOT match the pattern or patterns will be evaluated.
  ...
  @doc """
  Render the Liquid template to html.
  
  If the input template contains no Liquid tags, convert it to HTML at compile
  time. Otherwise, the returned macro will invoke Solid.render! at runtime to perform
  the coversion after evaluating the tags.
  """
  def solid_render(solid_content) do
    case solid_content do
      %Solid.Template{parsed_template: [%Solid.Text{}]} ->
        solid_content
        |> Solid.render!(%{})
        |> to_string()
        |> Phoenix.HTML.raw()
